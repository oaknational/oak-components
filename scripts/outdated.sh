...
function versionDistance(current, latest) {
  ...
  // Return [majorDiff, minorDiff, patchDiff] for sorting most-to-least out of date
...
process.stdin.on('end', () => {
  ...
  const rows = Object.entries(data)
    ...
    .sort((a, b) => {
      // First by category (major → minor → patch)
      const typeDiff = typeOrder[a.type] - typeOrder[b.type];
      if (typeDiff !== 0) return typeDiff;
      // Within category, most out of date first (descending by distance)
      for (let i = 0; i < 3; i++) {
