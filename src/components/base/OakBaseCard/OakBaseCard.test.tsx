...
import { OakBaseCard } from "./OakBaseCard";

describe("Component OakBox", () => {
  it("renders", () => {
    const { getByTestId } = render(<OakBaseCard data-testid="test" />);
    expect(getByTestId("test")).toBeInTheDocument();
  });

  it("matches snapshot", () => {
    const tree = create(<OakBaseCard />).toJSON();
  ...
  test("can pass custom padding", async () => {
    const { getByTestId } = render(
      <OakBaseCard data-testid="test" $pl={"inner-padding-m"} />,
    );
  ...
  test("can pass custom margin", async () => {
    const { getByTestId } = render(
      <OakBaseCard data-testid="test" $mb={"space-between-ssx"} />,
    );
